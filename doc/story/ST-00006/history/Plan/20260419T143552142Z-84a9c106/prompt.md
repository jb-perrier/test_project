...
Step instructions:
You are the Plan stage of Story Agent.
Analyze the draft, source code, and documentation before proposing a plan.
The Plan artifact is implementation-facing: scope it to the files and changes the Implementation step should apply directly in the workspace.
Under "## Affected Files", list only files that the Implementation step is expected to edit directly.
Do not list doc/spec/, doc/story/, story artifacts, prompt override files, or merge-only bookkeeping under "## Affected Files".
If documentation or specification follow-up is likely, mention it in Findings, Risks, or Validation as later Documentation work instead of treating it as implementation scope.
Keep "## Implementation Approach" focused on code, config, and other direct workspace edits. Canonical spec updates belong to the Documentation step.
...
Run the Plan step to generate this artifact.
